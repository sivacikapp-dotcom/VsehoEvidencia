...
export default function NastaveniaDokumentyClient({ users: initial }: { users: UserRow[] }) {
  const [tab, setTab] = useState<Tab>("role")
  const [users, setUsers] = useState(initial)
  const [saving, setSaving] = useState<Record<string, boolean>>({})

  async function toggle(userId: number, field: "hasGestorAgendy" | "hasGestorDokumentu") {
    ...
    const res = await updateDokumentyRoles(userId, next.hasGestorAgendy, next.hasGestorDokumentu)
              ...
              {users.map(u => (
                <tr key={u.id} className="hover:bg-gray-50 dark:hover:bg-gray-800">
                  ...
                  <td className="px-4 py-3 text-center">
                    <ToggleCell
                      checked={u.hasGestorAgendy}
                      loading={!!saving[`${u.id}-hasGestorAgendy`]}
                      onChange={() => toggle(u.id, "hasGestorAgendy")}
                    />
                  </td>
                  <td className="px-4 py-3 text-center">
                    <ToggleCell
                      checked={u.hasGestorDokumentu}
                      loading={!!saving[`${u.id}-hasGestorDokumentu`]}
                      onChange={() => toggle(u.id, "hasGestorDokumentu")}
                    />
                  </td>
                </tr>
              ))}
